...
void start(char *local_address, int port, char *dns_address) {
    int dtls_fd;
    union mysockaddr server_addr, client_addr, dns_addr;
#if WIN32
     WSADATA wsaData;
     DWORD tid;
#else
    ...
    */
    ...
    while(1) {
        ...
        while(current_session != NULL) {
            if(current_session->ssl_status == SSL_STATUS_ERR || (current_session->ssl_status == SSL_STATUS_OK && (SSL_get_shutdown(current_session->ssl) & SSL_RECEIVED_SHUTDOWN))) {
...
int main(int argc, char **argv) {
     int port = 853;
     char *local_addr = "::1";
     char *dns_addr = "8.8.8.8";
     start(local_addr, port, dns_addr);
     return 0;
}
